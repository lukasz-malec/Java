...
public class DataFrame {
    ...
    public Series apply(Applayable a, String name, int njobs) {
        // Divide the 'name' series into 'njobs' chunks
        List<Series> chunks = new ArrayList<>();
        int chunkSize = columns.get(0).values.size() / njobs;
        for (int i = 0; i < njobs; i++) {
            int start = i * chunkSize;
            int end = (i == njobs - 1) ? columns.get(0).values.size() : (i + 1) * chunkSize;
            Series chunk = new Series(columns.get(0).type, name + "_chunk" + i);
            for (int j = start; j < end; j++) {
                chunk.addValue(String.valueOf(columns.get(0).values.get(j)));
            }
            chunks.add(chunk);
        }

        // Apply the method from the interface 'a' to each chunk
        Series result = new Series(columns.get(0).type, name + "_result");
        for (Series chunk : chunks) {
            a.apply(chunk);
            result.values.addAll(chunk.values);
        }
